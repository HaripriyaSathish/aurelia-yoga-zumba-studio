        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {(plans || []).map((plan, i) => {
            ...
            return (
              ...
              >
                {plan.is_highlighted && (
                  <span className="absolute -top-3 left-1/2 -translate-x-1/2 flex items-center gap-1 px-3 py-1 rounded-full bg-white text-[#FF6B4A] text-[11px] font-bold uppercase tracking-wider shadow-lg">
                ...
                <div className="mt-4 flex items-baseline gap-1">
                  ...
                  {price > 0 && <span className={`text-sm ${plan.is_highlighted ? 'text-white/80' : 'text-[#A8A29A]'}`}>/{plan.duration}</span>}
                </div>
                {price === 0 && <p className={`text-sm mt-1 ${plan.is_highlighted ? 'text-white/80' : 'text-[#A8A29A]'}`}>{plan.duration}</p>}

                <ul className="mt-6 space-y-3 flex-1">
                  {(plan.features || []).map((feat, idx) => (
                    <li key={idx} className="flex items-start gap-2.5 text-sm">
                      <Check className={`w-4 h-4 mt-0.5 shrink-0 ${plan.is_highlighted ? 'text-white' : 'text-[#FF6B4A]'}`} />
                      <span className={plan.is_highlighted ? 'text-white/95' : 'text-[#A8A29A]'}>{feat}</span>
                    </li>
                  ))}
                </ul>
