...
export default function Testimonials({ testimonials }) {
  ...
  return (
    <section className="relative py-16 md:py-20 bg-[#0A1614] overflow-hidden">
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] rounded-full bg-[#1FBF8F]/5 blur-[120px]" />
      <div className="max-w-4xl mx-auto px-5 sm:px-8 relative">
        ...
        <div className="relative">
          <AnimatePresence mode="wait">
            ...
            >
              <Quote className="w-9 h-9 text-[#1FBF8F]/50 mb-4" />
              ...
              <div className="flex items-center gap-1 mt-6">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star key={i} className={`w-4 h-4 ${i < t.rating ? 'fill-[#1FBF8F] text-[#1FBF8F]' : 'text-[#3A3530]'}`} />
                ))}
              </div>

              <div className="mt-5 flex items-center gap-3">
                <img src={t.photo_url} alt={t.name} className="w-12 h-12 rounded-full object-cover border-2 border-[#1FBF8F]/40" />
                <div className="text-left">
                  <p className="font-display font-semibold text-[#FBF7F0]">{t.name}</p>
                  <p className="text-xs text-[#B9C7C2]">{t.program_joined}</p>
                </div>
              </div>
            </motion.div>
          </AnimatePresence>
              ...
              <div className="flex items-center gap-2">
                {list.map((_, i) => (
                  <button
                    key={i}
                    onClick={() => setIndex(i)}
                    className={`h-1.5 rounded-full transition-all duration-300 ${i === index ? 'w-6 bg-[#1FBF8F]' : 'w-1.5 bg-white/20'}`}
